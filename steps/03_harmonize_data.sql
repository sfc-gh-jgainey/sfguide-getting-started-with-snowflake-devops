...
create or replace view major_us_cities as select geo.geo_id, geo.geo_name, max(ts.value) total_population
  from cybersyn_government_essentials.cybersyn.datacommons_timeseries ts
  join cybersyn_government_essentials.cybersyn.geography_index geo on ts.geo_id = geo.geo_id
  join cybersyn_government_essentials.cybersyn.geography_relationships geo_rel on geo_rel.related_geo_id = geo.geo_id
  where true
